...
//added character
function Enigma(pos, ch){
  this.basepos = this.pos = pos.plus(new Vector(0.2,0.1));
  this.size = new Vector(0.8,0.8);
  this.wobble = Math.random() * Math.PI * 2;
}
Enigma.prototype.type = "Enigma";
...
Enigma.prototype.act = function(step){
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  if(obstacle){
    if(type = "lava" || "Enigma" &&this.status == null){
      this.status = 'lost';
      this.finishDelay = 1;
    }
  } else
...
Player.prototype.moveY = function(step, level, keys) {
  ...
  if (obstacle) {
    if(obstacle == "lava"|| "Enigma"&&this.status == null){
      this.status = 'lost';
...
if(type == 'Enigma'|| type == 'lava'&& this.status == null){
  this.status = "lost";
  this.finishDelay = 1;
